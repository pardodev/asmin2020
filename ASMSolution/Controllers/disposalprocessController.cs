...
{
    [Authorize]
    public class disposalprocessController : BaseController
    {
        ...
        public JsonResult GetDisposalList()
        {
            ...
            var _qry = (from dp in db.tr_disposal_announcement
                        where dp.deleted_date == null //&& dp.approval_disposal_type_id == 1
                        ...
                        (dp.fin_announcement_dept_id == UserProfile.department_id && dp.fl_fin_announcement == true))
        ...
        [ActionName("Process")]
        public ActionResult DisposalFollowup([Bind(Include = "announcement_id, request_id, announcement_upload_address, fl_fin_announcement, fin_announcement_upload_address, asset_number, asset_name, location_name, department_name, fl_SuggestionChanges, approval_suggestion_id")] disposalViewModel disposal_prs)
        {
            ...
            if (disposal_prs.fl_SuggestionChanges == null || disposal_prs.fl_SuggestionChanges == false)
            {
                ...
                if (disposal_announcement.fl_fin_announcement == null)
                {
                ...
                else if (disposal_announcement.fl_fin_announcement == true)
                {
                    if (Request.Files.Count > 0)
                    {
                        var file = Request.Files["fin_announcement_upload_address"];
                        if (file == null || file.ContentLength == 0)
                        {
                            ModelState.AddModelError("fin_announcement_upload_address", "Finance Document is mandatory.");
                        }
                    }
                }
            }
                    ...
                    try
                    {
                        if (disposal_prs.fl_SuggestionChanges == null || disposal_prs.fl_SuggestionChanges == false)
                        {
                            ...
                            if (disposal_announcement.fl_fin_announcement == null)
                            {
                                if (Request.Files.Count > 0)
                                {
                                    ...
                                    if (file != null && file.ContentLength > 0)
                                    {
                                        ...
                                        disposal_announcement.fl_fin_announcement = true;
                                        disposal_announcement.fin_announcement_dept_id = 2;
                                    }
                                }
                            }
                            else if (disposal_announcement.fl_fin_announcement == true)
                            {
                                if (Request.Files.Count > 0)
                                {
                                    ...
                                    if (file != null && file.ContentLength > 0)
                                    {
                                        var fileName = "dispose_Finance_" + disposal_prs.request_id.ToString() + "_" + Path.GetFileName(file.FileName);
                                        var path = Path.Combine(_file, fileName);
                                        file.SaveAs(path);

                                        disposal_announcement.fin_announcement_upload_address = fileName;
                                        disposal_announcement.fin_announcement_date = DateTime.Now;
                                        disposal_announcement.fin_announcement_emp_id = UserProfile.employee_id;
                                        isfin = true;
                            ...
                            disposal_announcement.fl_suggestion_changes = false;
                            ...
                            if (!isfin)
                            {
                                emailsetting = "EMAIL_TO_DISPOSAL_FINANCE";
                                to_name = "Department Finance";
                                freetext = "Procurement Department telah membuat <strong>Berita Acara</strong> dan menunggu tindaklanjut pembuatan <strong>Invoice</strong> oleh Finance Department.";
                            }
                            else if (isfin)
                            {
                                emailsetting = "EMAIL_TO_DISPOSAL_ACCOUNTING";
                                to_name = "Department Accounting";
                                freetext = "Dokumen <strong>Berita Acara</strong> dan <strong>Invoice</strong> sudah tersedia. Asset menunggu tindak lanjut (<strong>Dispose</strong>) oleh Accounting Department.";
                            }
                            ...
                            sy_email_log.elog_subject = string.Format("Asset Disposal Need Follow Up (Resale)");
                        ...
                        else if (disposal_prs.fl_SuggestionChanges == true)
                        {
                            ...
                            var refApproval = (from a in db.ms_job_level
                                               ...
                                               where (b.fl_active == true && b.deleted_date == null
                                                       && b.department_id == 4 && b.company_id == UserProfile.company_id)
            ...
            disposal_prs = DataDisposalView(disposal_prs.announcement_id, disposal_prs);
        ...
        private disposalViewModel DataDisposalView(int? id, disposalViewModel disposalmodel)
        {
            ...
            disposalmodel.fl_fin_announcement = _qry.tr_disposal_announcement.fl_fin_announcement;
            ...
            disposalmodel.disposal_type_list = (from dt in db.ms_disposal_type
                                                where dt.disposal_type_id != 1
                                                select dt).ToList();

            return disposalmodel;
        }
        public ActionResult ModalPDF()
        {
            return PartialView();
        }
    }
